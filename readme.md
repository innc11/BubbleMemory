...
借此方法可以在Typecho等编辑器进行编写文章，并且支持拖入图片，完全不用考虑资源引用的问题
...
主题自带一种永久链接的格式（占位符），生成的永久链接像这样`http://127.0.0.1:4000/20354-1804.html`

数字`20354-1804`代表2020年的第354天，后面的1804代表18点04分

使用方法：在Hexo`_config.yml`里设置`permalink: :plink.html`

其中`:plink`是占位符，会被替换成`20354-1804`这样的时间，后面的`.html`只是一个纯文本而已

尽量避免在同一分钟里连续发布两篇文章，会出现永久链接冲突
